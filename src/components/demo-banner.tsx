...
export function DemoBanner({ enabled = false }: DemoBannerProps) {
  ...
  /* min-h-10 rather than the caption's own box: these are the first two things
     a reviewer taps on a phone, so they get a real target instead of a chip. */
  const chip =
    "inline-flex min-h-10 items-center gap-1.5 rounded-input border border-line bg-canvas px-3 py-1 " +
    "text-caption text-muted transition-colors duration-150 ease-out-soft " +
    "hover:border-line-strong hover:text-ink active:scale-[0.98]";
      ...
      <div className="mx-auto flex w-full max-w-wide flex-wrap items-center gap-x-4 gap-y-2 px-4 py-2.5">
        ...
        <div className="ml-auto flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className="inline-flex min-h-10 min-w-10 items-center justify-center rounded-input px-3 py-1 text-caption text-faint transition-colors duration-150 ease-out-soft hover:text-ink active:scale-[0.98]"
          >
            Hide
          </button>
        </div>
